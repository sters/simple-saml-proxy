...
var errSAMLResponseNotFound = errors.New("SAMLResponse not found in HTML")
...
func fixSAMLResponse(htmlBody string) (string, error) {
	...
	if encodedResponse == "" {
		slog.Error("SAMLResponse not found in HTML", slog.String("html_snippet", htmlBody[:minInt(200, len(htmlBody))]))

		return "", errSAMLResponseNotFound
	}
	...
	slog.Info("Decoded SAML response XML",
		...
		slog.String("xml_preview", originalXML[:minInt(300, len(originalXML))]))
...
func (r *responseRecorder) Write(b []byte) (int, error) {
	...
	if err != nil {
		return n, fmt.Errorf("failed to write to response recorder: %w", err)
	}
